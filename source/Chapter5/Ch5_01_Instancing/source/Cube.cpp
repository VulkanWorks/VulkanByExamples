...
void Cube::CreateGraphicsPipeline()
{
    ...
    VkShaderModule vertShader = VulkanHelper::CreateShader(m_VulkanApplication->m_hDevice, "../source/shaders/CubeInstanceVert.spv"); // Relative path to binary output dir
    ...
    VkShaderModule fragShader = VulkanHelper::CreateShader(m_VulkanApplication->m_hDevice, "../source/shaders/CubeInstanceFrag.spv"); // Relative path to binary output dir
...
void Cube::prepareInstanceData()
{
	...
	InstanceBuffer.m_BufObj.m_MemoryFlags = memoryProperty;
	InstanceBuffer.m_BufObj.m_DataSize = instanceData.size() * sizeof(InstanceData);
